...
def find_best_move(board, max_depth):
    ...
    # Optimize: check fewer positions for faster response
    empty_cells = [(i, j) for i in range(len(board)) for j in range(len(board[0])) if board[i][j] == '']
    
    # If too many empty cells, sample randomly
    if len(empty_cells) > 15:
        empty_cells = random.sample(empty_cells, 15)
...
def ai_make_move(board, buttons, circle_image, square_image, player_turn, board_window, root,
    ...
    if player_turn[0] == 2:  # AI's turn (player 2)
        max_depth = 2  # Reduced for faster calculation
