...
def ai_make_move(board, buttons, circle_image, square_image, player_turn, board_window, root,
    ...
    if player_turn[0] == 2:  # AI's turn (player 2)
        ...
        if best_move != (-1, -1):
            row, col = best_move
            handle_click_ai(None, row, col, board, buttons, circle_image if best_char == 'S' else square_image,
                            square_image, player_turn, board_window, root, update_scoreboard,
            ...
            while player_turn[0] == 2:  # Check if it's still AI's turn
                ...
                if best_move != (-1, -1):
                    row, col = best_move
                    handle_click_ai(None, row, col, board, buttons, circle_image if best_char == 'S' else square_image,
                                    square_image, player_turn, board_window, root,
                                    update_scoreboard, check_winner, check_game_end, scoreboard_frame,
